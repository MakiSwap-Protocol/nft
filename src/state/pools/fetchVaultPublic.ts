...
export const fetchPublicVaultData = async () => {
  try {
    ...
    ] = await multicallv2(makiVaultAbi, calls);

    const totalSharesAsBigNumber = shares
      ? new BigNumber(shares.toString())
      : BIG_ZERO;
    const sharePriceAsBigNumber = sharePrice
      ? new BigNumber(sharePrice.toString())
      : BIG_ZERO;
    // const totalMakiInVaultEstimate = convertSharesToMaki(
    ...
    return {
      ...
      estimatedMakiBountyReward: new BigNumber(
        estimatedMakiBountyReward as string
      ).toJSON(),
      totalPendingMakiHarvest: new BigNumber(
        totalPendingMakiHarvest as string
      ).toJSON(),
    };
  } catch (error) {
    console.log("!!", error);

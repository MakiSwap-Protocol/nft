...
export const getCollectionApi = async (
  ...
  console.error(
    `API: Failed to fetch NFT collection ${collectionAddress}`,
    res.statusText
  );
  return null;
};
...
export const getNftsFromCollectionApi = async (
  ...
  const isPBCollection =
    collectionAddress.toLowerCase() === pancakeBunniesAddress.toLowerCase();
  ...
  console.error(
    `API: Failed to fetch NFT tokens for ${collectionAddress} collection`,
    res.statusText
  );
  return null;
};
      ...
      gql`
        query getNftCollectionMarketData($collectionAddress: String!) {
          collection(id: $collectionAddress) {
            id
            nfts(orderBy:${
              isPBCollection ? "updatedAt" : "tokenId"
...
export const getNftsByBunnyIdSg = async (
  ...
  try {
    ...
    console.error(
      `Failed to fetch collection NFTs for bunny id ${bunnyId}`,
      error
    );
    return [];
  }
...
export const getAllPancakeBunniesLowestPrice = async (
  ...
  try {
    ...
    const rawResponse: Record<string, { currentAskPrice: string }[]> =
      await request(
        GRAPH_API_NFTMARKET,
        gql`
    ...
    return Object.keys(rawResponse).reduce((lowestPricesData, subQueryKey) => {
      ...
      return {
        ...lowestPricesData,
        [bunnyId]:
          rawResponse[subQueryKey].length > 0
            ? parseFloat(rawResponse[subQueryKey][0].currentAskPrice)
            : Infinity,
      };
    }, {});
  } catch (error) {
  ...
  try {
    ...
    return Object.keys(rawResponse).reduce((updatedAtData, subQueryKey) => {
      ...
      return {
        ...
        [bunnyId]:
          rawResponse[subQueryKey].length > 0
            ? Number(rawResponse[subQueryKey][0].updatedAt)
            : -Infinity,
      };
    }, {});
  } catch (error) {
    console.error(
      "Failed to fetch PancakeBunnies latest market updates",
      error
    );
    return {};
  }
...
export const getLowestPriceInCollection = async (collectionAddress: string) => {
  try {
    ...
    console.error(
      `Failed to lowest price NFTs in collection ${collectionAddress}`,
      error
    );
    return 0;
  }
 ...
 * @returns NftTokenSg[]
 */
export const getLatestListedNfts = async (
  first: number
): Promise<TokenMarketData[]> => {
  try {
    const res = await request(
...
export const fetchNftsFiltered = async (
  ...
  const res = await fetch(
    `${API_NFT}/collections/${collectionAddress}/filter?${stringify(filters)}`
  );
...
export const getMetadataWithFallback = (
  apiMetadata: ApiResponseCollectionTokens["data"],
  bunnyId: string
) => {
...
export const fetchWalletTokenIdsForCollections = async (
  ...
  const walletNftPromises = map(
    collections,
    async (collection): Promise<TokenIdWithCollectionAddress[]> => {
      ...
      const getTokenId = async (index: number) => {
        try {
          const tokenIdBn: ethers.BigNumber =
...
export const combineCollectionData = (
  ...
  const collectionsMarketObj: Record<string, CollectionMarketDataBaseFields> =
    collectionSgData.reduce(
  ...
  return collectionApiData.reduce((accum, current) => {
    const collectionMarket =
      collectionsMarketObj[current.address.toLowerCase()];
...
export const combineNftMarketAndMetadata = (
  ...
  const completeNftData = nftsWithMetadata.map<NftToken>((nft) => {
    // Get metadata object
    const isOnSale =
      nftsForSale.filter((forSaleNft) => forSaleNft.tokenId === nft.tokenId)
        .length > 0;
    let marketData;
    if (isOnSale) {
      marketData = nftsForSale.find(
        (marketNft) => marketNft.tokenId === nft.tokenId
      );
    } else {
...
export const getCompleteAccountNftData = async (
  ...
  const uniqueCollectionAddresses = uniq(
    walletNftIdsWithCollectionAddress.map(
      (walletNftId) => walletNftId.collectionAddress
    )
  );

  const walletNftsByCollection = uniqueCollectionAddresses.map(
    (collectionAddress) => {
      return {
        collectionAddress,
  ...
  const walletMarketDataRequests = walletNftsByCollection.map(
    (walletNftByCollection) => {
      const tokenIdIn = walletNftByCollection.idWithCollectionAddress.map(
        (walletNft) => walletNft.tokenId
      );
 ...
 * @returns
 */
export const getCollectionDistributionApi = async <T>(
  collectionAddress: string
): Promise<T> => {
  const res = await fetch(
  ...
  console.error(
    `API: Failed to fetch NFT collection ${collectionAddress} distribution`,
    res.statusText
  );
  return null;
};

...
export function getEtherscanLink(
  ...
  const prefix = `https://${
    HUOBISCAN_PREFIXES[chainId] || HUOBISCAN_PREFIXES[128]
  }hecoinfo.com`;
...
export function getHuobiScanLinkForNft(
  collectionAddress: string,
  tokenId: string,
  chainId: ChainId = ChainId.MAINNET
): string {
  return `https://${HUOBISCAN_PREFIXES[chainId]}/token/${collectionAddress}?a=${tokenId}`;
}
...
export const getHourlyRateData = async (
  ...
  try {
    ...
    const timestamps = [];
    const timeDuration =
      startType === "second" ? 2 : startType === "minute" ? 60 : 3600;

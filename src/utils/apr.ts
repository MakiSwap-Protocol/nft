...
export const getFarmApr = (poolWeight: BigNumber, makiPriceUsd: BigNumber, poolLiquidityUsd: BigNumber): number => {
  // console.log('poolWeight', poolWeight.toString(), makiPriceUsd.toString(), poolLiquidityUsd.toString())
  const yearlyMakiRewardAllocation = MAKI_PER_YEAR.times(poolWeight)
  const apr = yearlyMakiRewardAllocation.times(makiPriceUsd).times(1000000).div(poolLiquidityUsd).div(10000)
  return apr.isNaN() || !apr.isFinite() ? null : apr.toNumber()
}
